...
// Structure for our custom EEPROM config
typedef union {
    uint32_t raw;
    struct {
        uint8_t lighting_mode : 2;  // 2 bits for lighting mode (3 possible values)
    };
} user_config_t;
...
//      Ct_L     Win_L    Alt_L                               SPACE                               Alt_R    FN       Ct_R     Left     Down     Right
    ...
    [1] = LAYOUT(
        _______, KC_MYCM, KC_WHOM, KC_CALC, KC_MSEL, KC_MPRV, KC_MNXT, KC_MPLY, KC_MSTP, KC_MUTE, KC_VOLD, KC_VOLU, _______, KC_INSERT,          _______,
...
// Save both QMK RGB settings and our custom lighting mode
void save_qmk_rgb_settings(void) {
    // Save QMK RGB settings
    ...
    // Save our lighting mode and user HSV values to EEPROM
    user_config.raw = 0;  // Clear the config
    user_config.lighting_mode = LIGHTING_MODE;
    
    // If in USER_SOLID_SIDE mode, save the user-defined HSV values
    if (LIGHTING_MODE == USER_SOLID_SIDE) {
        // Store user HSV values in the remaining bits of user_config
        user_config.raw |= (user_hsv.h << 8);
        user_config.raw |= (user_hsv.s << 16);
        user_config.raw |= (user_hsv.v << 24);
    }
...
void eeconfig_init_user(void) {
    user_config.raw = 0;  // Clear the config
    user_config.lighting_mode = QMK_FANCY_SIDE;  // Set default lighting mode
...
void keyboard_post_init_user(void) {
    ...
    // Try to load saved lighting mode and HSV values from EEPROM
    user_config.raw = eeconfig_read_user();
    LIGHTING_MODE = user_config.lighting_mode;
    
    // If in USER_SOLID_SIDE mode, restore the saved HSV values
    if (LIGHTING_MODE == USER_SOLID_SIDE) {
        user_hsv.h = (user_config.raw >> 8) & 0xFF;
        user_hsv.s = (user_config.raw >> 16) & 0xFF;
        user_hsv.v = (user_config.raw >> 24) & 0xFF;
    }
}
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (get_highest_layer(layer_state) > 20) {
        ...
        case KC_MUTE:
            if (record -> event.pressed) {
                if (get_mods() & MOD_BIT(KC_LCTL)) {
                    // If RGB is off, turn it on before cycling modes
